...
```bash
conda create -n llm-proxy python=3.13 -y
...
flowchart TD
    U[👤 用户] --> A[注册账号]
    A --> B{"REQUIRE_APPROVAL?"}
    B -->|"true"| C[等待管理员审批]
    B -->|"false（默认）"| D[直接登录]
    C --> D

    D --> E[🔧 创建代理]
    E --> F["输入目标 API 地址<br/>选择转发协议 HTTP/1.1 或 HTTP/2<br/>可选配置 API Key 覆盖<br/>（管理员可为其他用户创建）"]
    F --> G["系统分配 5 位编号<br/>（如 12345，管理员可后续编辑）"]

    G --> H["🔗 修改智能体 Base URL"]
    H --> I["原来: https://api.openai.com/v1/chat<br/>改为: http://IP:3998/12345/v1/chat"]
    ...
    L --> M["📊 查看详情页"]
    M --> N["实时查看交互记录<br/>展开 JSON / 树形查看<br/>导出全部数据"]

    style G fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style K fill:#d6eaf8,stroke:#2980b9,color:#154360
    style M fill:#fdebd0,stroke:#e67e22,color:#7e5100
```
...
```mermaid
flowchart TD
    subgraph Host["宿主机 (network_mode: host)"]
        subgraph Backend["llm-proxy-backend (Python)"]
            FASTAPI["FastAPI :3998<br/>管理 API + 共享代理"]
            PROXY["代理转发引擎<br/>httpx HTTP/1.1 + HTTP/2"]
            CLEANUP["后台清理 daemon 线程"]
        end

        subgraph Frontend["llm-proxy-frontend (nginx)"]
            NGINX["nginx :3999<br/>静态文件 + API 反向代理"]
            VUE["Vue 3 SPA<br/>Docker 构建阶段编译"]
        end
    end

    subgraph External["外部服务"]
        MYSQL[("MySQL<br/>独立部署")]
        LLM["上游 LLM API<br/>OpenAI / Anthropic / ..."]
    end

    Browser["浏览器 :3999"] -->|"HTTP"| NGINX
    NGINX -->|"/api/*"| FASTAPI
    NGINX -->|"静态文件"| VUE
    Agent["智能体 Agent"] -->|"POST /{port}/v1/..."| FASTAPI
    FASTAPI --> PROXY
    PROXY -->|"转发"| LLM
    FASTAPI --> MYSQL
    CLEANUP --> MYSQL
    ...
    style Host fill:#1a2a3a,stroke:#5dade2,color:#ecf0f1
    style Backend fill:#d6eaf8,stroke:#2980b9,color:#154360
    style Frontend fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style External fill:#fdebd0,stroke:#e67e22,color:#7e5100
```
...
└── frontend/                # Vue 3 前端
    ...
    └── src/
        ...
        └── views/
            ├── Login.vue          # 登录页
            ├── Register.vue       # 注册页
            ├── Dashboard.vue     # 代理列表 + 创建/编辑（含编号修改、管理员用户分配）
...
flowchart TD
    subgraph Entry["入口层"]
        MAIN["main.py<br/>FastAPI 应用 + lifespan<br/>路由注册 + 启动/关闭"]
    end

    subgraph Routing["路由层 (routers/)"]
        AUTH_R["auth_router.py<br/>注册/登录/密码修改"]
        ADMIN_R["admin_router.py<br/>用户管理 + 清理线程"]
        PORTS_R["ports_router.py<br/>端口 CRUD + 历史查询<br/>流式导出 + ticket"]
        CONFIG_R["config_router.py<br/>前端配置"]
    end
    ...
    subgraph Core["核心层"]
        ...
        MODELS["models.py<br/>ORM: User/Port/Request"]
        SCHEMAS["schemas.py<br/>Pydantic 模型"]
        CONFIG["config.py<br/>环境变量"]
    end

    subgraph Data["数据层"]
        DB["database.py<br/>三连接池 + 自动建库<br/>迁移 + 清理工具"]
    end
    ...
    style Entry fill:#d6eaf8,stroke:#2980b9,color:#154360
    style Routing fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style Proxy fill:#fdebd0,stroke:#e67e22,color:#7e5100
    style Core fill:#e8daef,stroke:#8e44ad,color:#4a235a
    style Data fill:#fadbd8,stroke:#e74c3c,color:#78281f
```
...
| 代理编辑 | 管理员可编辑代理编号（5 位数字）、目标地址、转发协议、API Key、**所属用户（可转移给其他用户）**；所有用户可通过独立的「🌐 可见性」开关切换 **public（所有用户可见）** |
...
| 轮询优化 | 新记录通过 `unshift()` 逐个插入而非重建数组，超出上限自动移除最旧记录 |
...
| DELETE | `/api/ports/{id}/history` | 清空历史（后台线程执行，立即返回） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/admin/users` | 用户列表（支持 `skip`/`limit` 分页参数，默认 0/2000） |
| PUT | `/api/admin/users/approve` | 审批用户 |
| DELETE | `/api/admin/users/{id}` | 删除用户（关联端口后台批量清理，立即返回） |
| GET | `/api/admin/deleted-ports` | 已删除代理列表（支持 `skip`/`limit` 分页参数，默认 0/5000） |
...
sequenceDiagram
    ...
    U->>P: POST /api/ports {target_url, description[, user_id]}
    P->>P: JWT 验证身份 (require_approved)
    alt user_id 已提供且是管理员
        P->>D: SELECT user WHERE id=? → 验证存在且已审批
        Note over P,D: 管理员可为其他用户创建代理
    else 普通用户或未提供 user_id
        P->>P: owner = current_user
    end
    P->>P: _validate_target_url() → SSRF 检查
    P->>D: SELECT GET_LOCK('llm_proxy_port_alloc')
    Note over P,D: MySQL 命名锁, 序列化分配
    P->>P: 随机生成 5 位数 (10000-99999)
    P->>D: SELECT 1 FROM ports WHERE port_number=? AND is_active=1 AND deleted_at IS NULL LIMIT 1
    Note over P,D: O(1) 碰撞检测，不加载全部端口号
...
sequenceDiagram
    ...
    P->>C: 返回完整响应（原始字节，与上游逐 bit 一致）
    
    P--)D: asyncio.create_task → 线程池 → LogSessionLocal → INSERT requests
    Note over P,D: 后台写入，不阻塞响应。日志存储使用 pretty-print JSON，<br/>但返回给客户端的响应体是上游原始字节。
```
...
| 字段 | 非流式 | 流式 |
|------|:--:|:--:|
| `port_id` | port.id 或 NULL | port.id 或 NULL（端口删除后仍保存，停用端口上的进行中请求也会保留记录） |
...
stateDiagram-v2
    ...
    note right of 清理中
        cleaning_started_at 设置时间戳
        daemon 线程批量 DELETE requests
        MySQL Event 每 30s 兜底
        清理完成后删除 port 行
    end note
...
**软删除机制**：删除端口时仅设置 `deleted_at` 时间戳和 `is_active=False`，不删除数据库记录。管理员可在「已删除代理」页面查看、恢复或彻底删除。彻底删除时采用后台批量清理架构（Python daemon 线程 + MySQL Event 双通道兜底），不会阻塞 HTTP 请求。软删除期间产生的交互记录仍写入 `requests` 表（`port_id=NULL`），数据不会丢失。

**停用 vs 软删除**：停用只是 `is_active=False`，代理编号保留且可重新启用。软删除后代理从缓存移除，编号不可再用。

## 数据模型

> `User.ports` 的 ORM cascade 设为 `save-update, merge, refresh-expire, expunge`（不含 delete/delete-orphan）。删除用户时不触发级联加载，改用后台批量清理架构（见[大量数据删除](#大量数据删除后台批量清理架构)）。

```mermaid
erDiagram
    users ||--o{ ports : "创建 (1:N)"
    ports ||--o{ requests : "产生 (1:N)"
    ...
    users {
        ...
        bool is_approved "已审批"
        datetime created_at "注册时间"
    }

    ports {
        int id PK "主键"
        int port_number UK "5 位随机编号"
        int user_id FK "所属用户"
        ...
        datetime cleaning_started_at "清理中标记"
    ...
    requests {
        ...
        string path "请求路径"
        longtext request_headers "请求头 JSON"
...
flowchart TD
    subgraph Transport["传输层"]
        TLS["HTTPS (nginx / Cloudflare)<br/>JWT + 密码加密传输"]
    end

    subgraph Auth["认证层"]
        JWT["JWT HS256<br/>7 天过期 + Bearer Token"]
        BCRYPT["bcrypt 密码哈希<br/>72 字节截断 + 随机盐"]
    end

    subgraph Access["访问控制层"]
        ROLE["role: admin / user<br/>依赖注入 require_admin<br/>require_approved"]
        APPROVAL["REQUIRE_APPROVAL 开关<br/>新用户注册审批控制"]
    end

    subgraph Gateway["网关层"]
        CORS["CORS 白名单<br/>可配置来源域名"]
        SSRF["SSRF 防护<br/>拦截内网 IP / localhost<br/>metadata 端点"]
    end

    subgraph Resource["资源层"]
        LOCK["MySQL GET_LOCK<br/>端口分配序列化互斥"]
        APIKEY["API Key 覆盖<br/>按端口替换认证头<br/>或原样透传"]
    end
    ...
    style Transport fill:#d6eaf8,stroke:#2980b9,color:#154360
    style Auth fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style Access fill:#fdebd0,stroke:#e67e22,color:#7e5100
    style Gateway fill:#e8daef,stroke:#8e44ad,color:#4a235a
    style Resource fill:#fadbd8,stroke:#e74c3c,color:#78281f
```
...
sequenceDiagram
    ...
    DB-->>API: {id, password_hash, role, is_approved}
    API->>API: bcrypt.checkpw(密码, hash)
    API->>API: 检查 is_approved 或 role=admin
    API->>API: create_access_token({user_id, role})
    API-->>F: {access_token, username, role}
    F->>F: localStorage 存储 token

    Note over U,DB: 后续请求
    U->>F: 访问任意页面
    F->>API: GET /api/ports<br/>Authorization: Bearer <token>
    API->>API: jwt.decode(token, SECRET_KEY)
    API->>DB: SELECT user WHERE id=?
    DB-->>API: User 对象
    API->>API: require_approved / require_admin 检查
    API-->>F: 受保护的资源
```
...
```mermaid
sequenceDiagram
    actor C as 智能体 (N 个并发)
    participant API as FastAPI (单线程 asyncio)
    participant CACHE as 端口缓存 (内存)
    participant HTTPX as httpx 连接池
    participant UP as 上游 LLM API
    participant THREAD as DB 写线程池 (8 线程)
    participant MYSQL as MySQL

    par 并发请求 1
        C->>API: POST /12345/v1/chat
        API->>CACHE: aget_target_url(12345)
        CACHE-->>API: target_url (微秒级)
        API->>HTTPX: client.stream()
        HTTPX->>UP: HTTP/1.1 转发 (keepalive 复用)
        UP-->>HTTPX: SSE 流 (数秒~数分钟)
        loop 逐 chunk
            HTTPX-->>API: chunk
    ...
    and 并发请求 2
        ...
        API->>HTTPX: client.stream() (另一条连接)
        HTTPX->>UP: HTTP/1.1 转发
        UP-->>HTTPX: 非流式 JSON 响应
        HTTPX-->>API: 完整响应
        API-->>C: 返回 JSON
        API--)THREAD: create_task(save_record)
        THREAD->>MYSQL: INSERT requests
    end
```
...
删除用户或清空端口历史时，关联的 `requests` 表可能包含数百万行 LONGTEXT 数据。如果在 HTTP 请求处理线程中同步执行 `DELETE`，会导致请求超时、事件循环阻塞，甚至 OOM。

本系统采用**双通道后台批量清理**架构——所有大量删除操作立即返回 HTTP 响应，真正的数据清理在后台 daemon 线程中分批执行，MySQL Event 作为兜底保障。
...
flowchart TD
    A[用户/管理员发起删除] --> B{操作类型}
    B -->|"彻底删除端口<br/>DELETE /api/admin/ports/{id}/permanent"| C1["设置 cleaning_started_at 标志<br/>立即返回 202"]
    B -->|"删除用户<br/>DELETE /api/admin/users/{id}"| C2["遍历用户所有 port<br/>逐一标记 cleaning_started_at<br/>立即返回 200"]
    B -->|"清空历史<br/>DELETE /api/ports/{id}/history"| C3["立即返回 200<br/>后台 daemon 线程启动"]

    C1 & C2 --> D["_spawn_cleanup_thread()<br/>daemon 线程批量 DELETE<br/>BATCH_SIZE=1000, 每10批 sleep 0.1s"]
    C3 --> E["_do_history_cleanup()<br/>daemon 线程批量 DELETE<br/>BATCH_SIZE=1000, 每10批 sleep 0.1s"]

    D --> F{"进程存活?"}
    F -->|"是"| G["线程删完 requests<br/>→ 删除 port 行<br/>→ 完成 ✔️"]
    F -->|"否（崩溃/重启）"| H["resume_port_cleanups()<br/>启动时扫描 cleaning_started_at<br/>重新 spawn daemon 线程"]
    H --> D

    D & E --> I["MySQL Event<br/>evt_cleanup_flagged_ports<br/>每 30 秒执行一次"]
    I --> J{"port.cleaning_started_at<br/>IS NOT NULL?"}
    J -->|"是"| K["DELETE FROM requests<br/>WHERE port_id=? LIMIT 1000"]
    J -->|"否"| I
    K --> L{"ROW_COUNT() = 0?"}
    L -->|"是"| M["DELETE FROM ports<br/>WHERE id=?"]
    L -->|"否"| I

    style I fill:#1a2a3a,stroke:#5dade2,color:#5dade2
    style H fill:#3a2a1a,stroke:#f39c12,color:#f1c40f
    style G fill:#1a3a1a,stroke:#2ecc71,color:#2ecc71
```

### 通道 1：Python daemon 线程（快速通道）

每个待清理的端口独立启动一个 daemon 线程，使用专用 `pymysql` 长连接（`read_timeout=3600`, `write_timeout=3600`）。

**共享基础设施**（`database.py`）：

- `get_raw_connection()` — 创建 1 小时超时的专用 pymysql 连接，供 `admin_router.py` 和 `ports_router.py` 的清理线程共用
- `warn_fragmented_raw()` — 批量删除后检测 `requests` 表碎片，`data_free/total > 30%` 时提醒 `OPTIMIZE TABLE`（异常不再静默吞掉，输出 `exc_info=True`）

**清理循环**：

```
while True:
    DELETE FROM requests WHERE port_id=? LIMIT 1000
    if rowcount == 0: break
    commit()
    if batch_num % 10 == 0: sleep(0.1)   # 节流，避免 MySQL I/O 过载
DELETE FROM ports WHERE id=?             # 仅彻底删除时执行
```

| 特性 | 实现 |
|------|------|
| 批次大小 | `CLEANUP_BATCH_SIZE`（默认 1000 行/批），可通过 `.env` 调整 |
| 日志间隔 | 每 `CLEANUP_LOG_INTERVAL`（默认 10）批输出进度日志 |
| 节流 | 每 N 批 `sleep(0.1)`，减轻 MySQL 磁盘 I/O 压力 |
| 连接隔离 | 独立 pymysql 连接（`get_raw_connection()`），不占用 SQLAlchemy 连接池 |
| 去重（端口级） | `_cleanup_lock` + `_cleanup_threads` dict，同一 port 不会启动两个清理线程 |
| 去重（历史级） | `_history_cleanup_lock` + `_history_cleanup_threads` dict，清空历史不可重复触发 |
| 并发限流 | `BoundedSemaphore` 限制同时运行的清理线程数（端口级 5、历史级 3），防止删除大量端口时耗尽 MySQL 连接 |
| 碎片化检测 | `warn_fragmented_raw()` — 清理完成后检测碎片并提醒运维 |
| commit 安全 | `delete_user` 中先 `db.commit()` 持久化标记，再 spawn 线程 — 事务回滚不会留下孤儿线程 |
| 日志完备 | 所有清理阶段均有结构化日志（spawn/进度/完成/失败），异常不静默吞掉 |

### 通道 2：MySQL Event（兜底通道）
...
DO
BEGIN
    -- 找一个标记了 cleaning_started_at 的端口
...
MySQL Event 在 Python 进程**完全不存在**的情况下依然运行——即使进程崩溃且永不重启，MySQL 服务端也会每 30 秒蚕食 1000 行，直到数据全部清除。

### 启动恢复

`main.py` 的 `lifespan` 启动阶段调用 `resume_port_cleanups()`：
...
进程重启后，所有未完成的清理任务自动恢复。MySQL Event 在重启间隙继续工作，两者互补。

### 三种删除操作的对比

| 操作 | API | 删除范围 | 立即返回 | 后台通道 | 兜底 |
|------|-----|----------|:--:|------|:--:|
| 软删除端口 | `DELETE /api/ports/{id}` | 仅标记 `deleted_at`，不删数据 | ✅ | 无 | 无 |
| 清空历史 | `DELETE /api/ports/{id}/history` | 删除该端口全部 requests | ✅ | daemon 线程 | 无（进程挂了可重试） |
| 彻底删除端口 | `DELETE /api/admin/ports/{id}/permanent` | 删除 requests + port | ✅ | daemon 线程 | MySQL Event |
| 删除用户 | `DELETE /api/admin/users/{id}` | 删除 user + 级联清理所有 port | ✅ | daemon 线程 × N | MySQL Event |

### 级联删除改造

`User` 和 `Port` 之间原本使用 SQLAlchemy 的 `cascade="all, delete-orphan"`，删除用户时 ORM 会将所有关联的 `Port` 和 `Request` 加载到内存再逐条 DELETE。改为 `cascade="save-update, merge, refresh-expire, expunge"` 后：

1. 删除用户时不再自动加载子行
2. 遍历用户的 port，逐一标记 `cleaning_started_at` 等字段 → **先 `db.commit()` 持久化** → 再 spawn daemon 线程
3. User 行立即删除，端口和请求在后台逐步清理

> **关键安全设计**：线程在 commit 之后启动。如果 commit 失败（事务回滚），`cleaning_started_at` 不会被持久化，线程启动后第一步检查 `SELECT id FROM ports` 会发现 port 未被标记，立即退出 — 不会误删正常数据。
...
sequenceDiagram
    ...
    A->>API: DELETE /api/admin/users/42
    API->>DB: SELECT ports WHERE user_id=42 AND cleaning_started_at IS NULL
    DB-->>API: [port 100, port 200, port 300]

    Note over API,DB: ① 标记阶段（仍在事务中）
    loop 每个 port
        API->>DB: UPDATE port SET cleaning_started_at=NOW(), is_active=0, deleted_at=NOW()
    end
    API->>DB: DELETE FROM users WHERE id=42
    API->>DB: COMMIT ✅

    Note over API,T: ② spawn 阶段（commit 成功后）
    API-->>A: 200 {"message": "User deleted. 3 port(s) cleaning in background."}
    loop 每个 port
        API->>T: _spawn_cleanup_thread(port.id)
    end

    par 后台并行清理
        T->>DB: DELETE FROM requests WHERE port_id=100 LIMIT 1000
        T->>DB: DELETE FROM requests WHERE port_id=200 LIMIT 1000
        T->>DB: DELETE FROM requests WHERE port_id=300 LIMIT 1000
    end
```
...
flowchart LR
    subgraph Input["请求进入"]
        REQ["智能体发送原始请求<br/>含所有 HTTP 头"]
    end

    subgraph Record["记录阶段"]
        DB_SAVE["全部原始头 → 序列化<br/>→ request_headers 字段<br/>（永不丢失）"]
    end

    subgraph Filter["转发过滤"]
        DIR["移除 6 个头<br/>host / content-length<br/>connection / transfer-encoding<br/>content-encoding<br/>accept-encoding"]
        HOST["替换 host 为目标 API"]
        AUTH_CHECK{"端口配置了<br/>api_key?"}
    end

    subgraph Forward["转发阶段"]
        PASSTHROUGH["原样透传<br/>Authorization / x-api-key 等"]
        OVERRIDE["替换所有认证头<br/>为系统配置的 api_key<br/>（若原请求无认证头则自动添加）"]
    end
    ...
    DIR --> HOST
    HOST --> AUTH_CHECK
    AUTH_CHECK -->|"否（NULL）"| PASSTHROUGH
    AUTH_CHECK -->|"是（已配置）"| OVERRIDE

    style Record fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style Filter fill:#fdebd0,stroke:#e67e22,color:#7e5100
    style Forward fill:#d6eaf8,stroke:#2980b9,color:#154360
```
...
每个到达 `/{port_number}/{path}` 的请求都经过以下决策链：

```mermaid
flowchart TD
    REQ["请求到达<br/>POST /12345/v1/chat/completions"] --> CACHE{"端口缓存命中?"}
    CACHE -->|"是 (99.9%)"| TARGET["获取 (target_url, prefer_http2, api_key)"]
    CACHE -->|"否 / 过期"| DB_LOOKUP["DB 查询 + 刷新缓存"]

    DB_LOOKUP --> EXISTS{"端口存在<br/>且活跃?"}
    EXISTS -->|"是"| TARGET
    EXISTS -->|"否"| ERR404["404 Not Found"]

    TARGET --> BUILD["构建转发 URL<br/>target_url + path + query"]
    BUILD --> HEADERS["处理请求头<br/>移除 6 个 + 替换 host<br/>API Key 覆盖检查"]

    HEADERS --> READ_BODY["SpooledTemporaryFile<br/>读取请求体<br/>≤10MB 内存 / >10MB 磁盘"]

    READ_BODY --> PROTO{"prefer_http2?"}
    PROTO -->|"HTTP/1.1"| H1["httpx HTTP/1.1 客户端<br/>独立 TCP 连接"]
    PROTO -->|"HTTP/2"| H2["httpx HTTP/2 客户端<br/>多路复用连接"]

    H1 --> STREAM{"响应 Content-Type?"}
    H2 --> STREAM

    STREAM -->|"text/event-stream"| SSE["SSE 流式处理<br/>逐 chunk 透传 + 缓冲<br/>流结束 → 重建 JSON → 写 DB"]
    STREAM -->|"其他"| NONSTREAM["非流式处理<br/>完整读取响应<br/>直接写 DB"]

    SSE --> DONE["返回 StreamingResponse"]
    NONSTREAM --> DONE2["返回 Response"]

    style CACHE fill:#d5f5e3,stroke:#27ae60,color:#145a32
    style SSE fill:#d6eaf8,stroke:#2980b9,color:#154360
    style ERR404 fill:#fadbd8,stroke:#e74c3c,color:#78281f
```
